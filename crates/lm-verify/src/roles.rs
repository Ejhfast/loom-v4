...
pub(crate) fn verify_core_roles(module: &Module) -> Result<(), VerifyError> {
    ...
    for (role, name) in [
        (lm_bytecode::corepin::ROLE_MODULE_CODE, "ModuleCode"),
        (
            lm_bytecode::corepin::ROLE_DECLARATION_CODE,
            "DeclarationCode",
        ),
        (lm_bytecode::corepin::ROLE_MEMBER_CODE, "MemberCode"),
        (lm_bytecode::corepin::ROLE_OPEN_CODE, "OpenCode"),
    ] {
        let Some(idx) = slot(role) else { continue };
        let class = &module.classes[idx as usize];
        if class.kind != BcClassKind::Normal
    ...
    for (role, name, arity) in [
        ...
        (lm_bytecode::corepin::ROLE_FUNCTION_DEF, "FunctionDef", 1),
