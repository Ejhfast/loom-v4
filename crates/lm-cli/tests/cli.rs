...
fn run_reports_a_fault_with_a_stable_code() {
    ...
    assert_eq!(
        stdout(&out),
        "Fault(DivideByZero)\n  at <entry> (tests/run-fault/divide-by-zero.lm:1:1, bytecode 2, ec074e0b)\n"
    );
}
...
fn run_with_a_small_fuel_budget_faults_with_out_of_fuel() {
    ...
    assert_eq!(
        stdout(&out),
        "Fault(OutOfFuel)\n  at <entry> (examples/01-basics/control.lm:2:1, bytecode 3, a406c51d)\n"
    );
}
...
fn inspect_dumps_the_checkpoint_container() {
    ...
    assert!(dump.contains("format 38 abi "), "{dump}");
