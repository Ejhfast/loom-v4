...
fn table_pass_charges_the_granter_row() {
    ...
    assert_eq!(
        code_of("def f(vm: Run[Int]) with Vm\n  vm.table().pass(Io)\nend\n1\n"),
        "E1046"
    );
...
fn mock_needs_the_exact_pure_signature() {
    ...
    assert_eq!(
        code_of(
            "def f(vm: Run[Int]) with Vm\n  vm.table().mock(Clock.Now, { ||: Bool true })\nend\n1\n"
        ),
        "E1004"
    );
...
fn a_label_must_name_a_parameter_of_the_target() {
    // `args` is a parameter of `activate` alone. Another target does
    // not gain the name.
    assert_eq!(
...
fn mock_runs_pure_and_bounded() {
    let source = "def go(): Int with Vm\n  \
        ...
        vm.table().mock(Clock.Now, { ||: Int 41 })\n  \
        case vm.run()\n  in Ok(v) then v\n  in Err(_) then -1\n  end\nend\ngo()\n";
    ...
    let source = "def go(): String with Vm\n  \
        vm = sys.vm.Vm().activate_or_fault(do || with Clock.Now\n    sys.clock.now() + 1\n  end, args: ())\n  \
        vm.table().mock(Clock.Now, { ||: Int 1 / 0 })\n  \
        case vm.run()\n  in Ok(_) then \"done\"\n  in Err(f) then f.code()\n  end\nend\ngo()\n";
    ...
    let source = "def go(): Int with Vm\n  \
        base = 40\n  \
        vm = sys.vm.Vm().activate_or_fault(do || with Clock.Now\n    sys.clock.now() + 2\n  end, args: ())\n  \
        vm.table().mock(Clock.Now, { ||: Int base })\n  \
        case vm.run()\n  in Ok(v) then v\n  in Err(_) then -1\n  end\nend\ngo()\n";
    assert_eq!(allowed(source, &["Vm"]), "Done(42)");
    ...
    let source = "def go(): Int with Vm\n  \
        xs = [7]\n  \
        vm = sys.vm.Vm().activate_or_fault(do || with Clock.Now\n    sys.clock.now()\n  end, args: ())\n  \
        vm.table().mock(Clock.Now, { ||: Int xs.len() + 40 })\n  \
        xs.push(1)\n  \
        case vm.run()\n  in Ok(v) then v\n  in Err(_) then -1\n  end\nend\ngo()\n";
...
fn one_vm_activates_runs_with_two_result_types() {
    let source = "def go(): (Int, String) with Vm\n  image = sys.vm.Vm()\n  \
        numbers = image.activate_or_fault({ || 7 }, args: ())\n  \
        text = image.activate_or_fault({ || \"ready\" }, args: ())\n  \
        n = case numbers.run()\n  in Ok(v) then v\n  in Err(_) then -1\n  end\n  \
        s = case text.run()\n  in Ok(v) then v\n  in Err(_) then \"fault\"\n  end\n  \
        (n, s)\nend\ngo()\n";
...
fn terminal_execution_calls_are_idempotent() {
    let source = "def go(): Int with Vm\n  \
        vm = sys.vm.Vm().activate_or_fault({ || 21 }, args: ())\n  \
        first = case vm.run()\n  in Ok(v) then v\n  in Err(_) then -1\n  end\n  \
        second = case vm.run()\n  in Ok(v) then v\n  in Err(_) then -1\n  end\n  \
        third = case vm.drive()\n  in Done(v) then v\n  in Asked(_) then -2\n  in Fault(_) then -1\n  end\n  \
        first + second + third\nend\ngo()\n";
...
fn cross_vm_tokens_fault_safely() {
    let source = "def spawn_now(): Run[Int] with Vm\n  \
        sys.vm.Vm().activate_or_fault(do || with Clock.Now\n    sys.clock.now()\n  end, args: ())\nend\n\
        def go(): Int with Vm\n  \
        a = spawn_now()\n  b = spawn_now()\n  \
        case a.drive()\n  in Asked(qa)\n    \
        case b.drive()\n    in Asked(qb)\n      \
...
fn reject_installs_the_supplied_fault() {
    let source = "def go(): String with Vm\n  \
        ...
        case blocked.run()\n  in Ok(_) then \"no-fault\"\n  in Err(fault)\n    \
        case vm.drive()\n    in Asked(q)\n      \
        vm.reject(q, fault)\n      \
        case vm.run()\n      in Ok(_) then \"done\"\n      in Err(f2) then f2.code()\n      end\n    \
...
fn dispatch_applies_the_controlled_table() {
    let source = "def go(): Int with Vm\n  \
        vm = sys.vm.Vm().activate_or_fault(do || with Clock.Now\n    sys.clock.now() + 1\n  end, args: ())\n  \
        vm.table().mock(Clock.Now, { ||: Int 10 })\n  \
        case vm.drive()\n  in Asked(q)\n    \
        vm.dispatch(q)\n    \
        case vm.run()\n    in Ok(v) then v\n    in Err(_) then -1\n    end\n  \
        in Done(_) then -2\n  in Fault(_) then -3\n  end\nend\ngo()\n";
...
fn routed_reject_faults_the_performing_descendant() {
    let source = r#"
def reject_print(vm: Run[String], source_fault: Fault): String with Vm
  loop do
    case vm.drive()
    in Asked(q)
      case q
...
fn every_ancestor_still_authorizes_a_routed_request() {
    let source = r#"
def drive_without_print(vm: Run[Int]): Int with Vm
  loop do
    case vm.drive()
    in Asked(q)
      case q
...
fn an_ancestor_mock_resolves_before_its_driver() {
    let source = r#"
def drive_without_clock(vm: Run[Int]): Int with Vm
  loop do
    case vm.drive()
    in Asked(q)
      case q
...
fn a_routed_token_rejects_another_surface_machine() {
    let source = r#"
def answer_through_wrong_vm(vm: Run[Int], wrong: Run[Int]): Int with Vm
  loop do
    case vm.drive()
    in Asked(q)
      case q
...
fn a_nested_step_surfaces_a_descendant_request() {
    let source = r#"
def drive_loop(vm: Run[Int], mut seen: [String]): Int with Vm
  loop do
    case vm.drive()
    in Asked(q)
      case q
...
def step_all(b: Run[Int]): Int with Vm
  loop do
    case b.step()
    in Ran     then ()
    in Waiting then ()
...
def a_drives_b(b: Run[Int]): Int with Vm
  loop do
    case b.drive()
    in Asked(q)
      case q
...
fn terminal_results_cross_the_boundary_as_a_copy() {
    ...
    let source = "def go(): Int with Vm\n  \
        vm = sys.vm.Vm().activate_or_fault(do ||\n    xs = [1, 2, 3]\n    xs.freeze()\n  end, args: ())\n  \
    ...
    let source = "def go(): String with Vm\n  \
        vm = sys.vm.Vm().activate_or_fault(do ||: Vm with Vm\n    sys.vm.Vm()\n  end, args: ())\n  \
        vm.table().pass(Vm)\n  \
        case vm.run()\n  in Ok(_) then \"done\"\n  in Err(f) then f.code()\n  end\nend\ngo()\n";
...
fn op_name_takes_no_arguments() {
    let source = "def go(): String with Vm\n  \
        vm = sys.vm.Vm().activate_or_fault({ ||: Int 1 }, args: ())\n  \
