...
#[test]
fn content_coding_helpers_transform_requests_and_responses() {
    let source = r#"
use std.compress.CompressionLevel
use std.http.HttpContentCoding
...
request = HttpRequest(
  "POST",
  "/data",
  [HttpHeader("X-Test", b"yes")],
  b"loom loom loom loom"
)
encoded_request = http.encode_request_content(
  request,
  HttpContentCoding.Deflate,
  CompressionLevel.Fast,
  limits
).expect("the request encodes")
decoded_request = http.decode_request_content(encoded_request, limits).expect("the request decodes")
response = HttpResponse(200, [], b"reply reply reply")
encoded_response = http.encode_response_content(
  response,
  HttpContentCoding.Gzip,
  CompressionLevel.Balanced,
  limits
).expect("the response encodes")
decoded_response = http.decode_response_content(encoded_response, limits).expect("the response decodes")
(
  encoded_request.header("content-encoding").expect("header").text(),
  decoded_request.body.text(),
  decoded_request.headers.len(),
  encoded_response.body.at(0),
  encoded_response.body.at(1),
  decoded_response.body.text(),
  decoded_response.headers.len()
)
"#;

    assert_eq!(
        run(source),
        "Done((\"deflate\", \"loom loom loom loom\", 1, 31, 139, \"reply reply reply\", 0))"
    );
}

#[test]
fn parsing_preserves_encoded_bytes_until_explicit_decoding() {
    let source = r#"
use std.compress.CompressionLevel
...
http = Http()
limits = http.default_limits()
compressed = gzip_compress(b"wire body", CompressionLevel.Fast)
response = HttpResponse(
  200,
  [HttpHeader("Content-Encoding", b"gzip")],
  compressed
)
wire = http.serialize_response(response, "OK", limits).expect("the response serializes")
parsed = http.parse_response(wire, "GET", limits).expect("the response parses")
decoded = http.decode_response_content(parsed, limits).expect("the response decodes")
(parsed.body == compressed, decoded.body.text(), decoded.headers.len())
"#;

    assert_eq!(run(source), "Done((true, \"wire body\", 1))");
}

#[test]
fn content_decoding_rejects_ambiguous_invalid_and_large_values() {
    let source = r#"
use std.compress.CompressionLevel
use std.compress.gzip_compress
...
duplicate_result = case http.decode_response_content(duplicate, limits)
in Err(error) then display(error)
...
invalid_result = case http.decode_request_content(invalid, limits)
in Err(error) then display(error)
in Ok(_) then "accepted"
end
compressed = gzip_compress(b"12345", CompressionLevel.Fast)
small = HttpLimits(1024, 16, 4, 16, 1024, 64)
large = HttpResponse(200, [HttpHeader("Content-Encoding", b"gzip")], compressed)
large_result = case http.decode_response_content(large, small)
in Err(error) then display(error)
in Ok(_) then "accepted"
end
(duplicate_result, invalid_result, large_result)
"#;

    assert_eq!(
        run(source),
        concat!(
            "Done((\"the HTTP content encoding is duplicated\", ",
            "\"the compressed HTTP body is invalid\", ",
            "\"the decoded HTTP body is too large\"))"
        )
    );
}
